...
- the drill output is the evidence; the archive alone is only a backup. This document sets that distinction, and the reference tools do not enforce it: the minimum-readiness gate accepts an archive-only evidence reference the same way it accepts a drill result, so the difference shows up in what a later reviewer can rely on;
