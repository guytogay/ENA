...
def refuse(message: str) -> int:
    """Emit one controlled ENA refusal without disguising unexpected defects."""
...
def main() -> int:
    ...
    # Validate every stable/verification setting and the target itself before
    # creating directories or files. A rejected caller assertion must not leave
    # a half-initialized home.
    if args.system_valid_hours <= 0:
        return refuse("--system-valid-hours must be > 0")

    language_problem = language_tag_problem(args.language)
    if language_problem:
        return refuse(f"--language: {language_problem}")
    ...
    if not args.verified_minimum and (args.recovery_evidence is not None or args.rescuer_evidence is not None):
        return refuse("--recovery-evidence/--rescuer-evidence require --verified-minimum")
    ...
    if args.verified_minimum:
        ...
        if recovery_evidence_issue:
            return refuse(f"--recovery-evidence {recovery_evidence_issue}")
        if rescuer_evidence_issue:
            return refuse(f"--rescuer-evidence {rescuer_evidence_issue}")

    home = Path(args.home).expanduser().resolve()
    if home.exists() and not home.is_dir():
        return refuse(f"Refusing to initialize ENA home because target is not a directory: {home}")

    config = home / "ENA.yaml"
    system = home / "SYSTEM.yaml"
    for path in (config, system):
        if path.exists():
            return refuse(f"Refusing to overwrite existing {path}")
    ...
    try:
        ...
        return refuse(f"Cannot initialize ENA home {home}: {exc}")
