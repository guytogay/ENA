...
### Added
- `tools/ena_peer_effect.py` and the two A2A effect-observation sections in `A2A.md` (#88): the missing half of dispatched work. Attribution already answered *who initiated* a peer task; nothing answered *what it did*. On a real Host a dispatched task created one file and deleted another, the child session replied only `DONE`, and no durable receiving-side record contained the task id or either path — the only trace of the deletion was the missing file. The helper snapshots a declared scope before dispatch, re-observes it after the dispatched process exits, derives `added` / `removed` / `modified` from content digests, and writes a durable record plus a one-line receipt keyed by the same correlation id. Its limits are enforced rather than disclaimed: the scope is an observation boundary and never an authorization boundary, the tool never refuses work for an unexpected effect, symlinks are recorded and not followed, and an incomplete observation exits `3` with the record marking itself incomplete instead of producing a false empty result. This is a deliberate net-growth exception under the contract-surface budget — it retires no field, and what it replaces is the habit of treating a child session's `DONE` as effect evidence. Costs are stated in the issue: bounded tree hashing is proportional to the declared scope, effects outside it stay invisible unless the Host observes them, and useful neighbouring changes must not be refused for being unpredicted.
